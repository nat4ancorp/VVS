	...
	switch(what){
		...
		case "type":
			if(state=="focus"){
				...
				if(currentType=="upload"){
					document.getElementById("placeholder-type-"+placeholder_id).innerHTML = "<select name=\"new_type\" id=\"new_type\" size=\"1\"><option value=\"upload\" selected=\"selected\">Upload</option><option value=\"youtube\">Youtube</option></select><a onclick=\"edit("+placeholder_id+",'type','blur')\" style=\"cursor:pointer;\">[X]</a>";
				} else if(currentType=="youtube"){
					document.getElementById("placeholder-type-"+placeholder_id).innerHTML = "<select name=\"new_type\" id=\"new_type\" size=\"1\"><option value=\"upload\">Upload</option><option value=\"youtube\" selected=\"selected\">Youtube</option></select><a onclick=\"edit("+placeholder_id+",'type','blur')\" style=\"cursor:pointer;\">[X]</a>";
				}
			} else if(state=="blur"){
				var currentType = document.getElementById("placeholder-currentType-"+placeholder_id).value;
		...
		case "story":
			if(state=="focus"){
				...
				if(currentStory==""){document.getElementById("placeholder-story-"+placeholder_id).innerHTML = "[<a onclick=\"edit("+placeholder_id+",'story','focus');\">Add a Story</a>]";}else{document.getElementById("placeholder-story-"+placeholder_id).innerHTML = "<a onclick=\"edit("+placeholder_id+",'story','focus');\" title="+currentStory+"\">"+currentStory+"</a>";}
			}
		break;
	}
}
...
function del(entry_id){
	...
	var queryString = "?action=del&entry_id=" + entry_id;
	ajaxRequest.open("GET", "update.php" + queryString, true);
	ajaxRequest.send(null); 
}
